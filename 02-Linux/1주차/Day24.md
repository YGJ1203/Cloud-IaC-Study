...
💡 Linux에서 길을 잃었다면 가장 먼저 pwd로 현재 위치를 확인한다.
...
현재 위치와 관계없이 항상 /etc/httpd/conf.d로 이동한다.
...
linux, test가 존재하지 않아도 한 번에 전체 구조를 생성할 수 있다.
...
→ test.txt의 시간을 2026-08-10 15:00으로 지정
...
권한, 소유권, 시간 정보 등을 가능한 한 보존하여 복사

-r

디렉터리와 하위 내용을 재귀적으로 복사
...
절대 경로는 /부터, 상대 경로는 현재 위치부터 시작한다.

ls 옵션은 처음부터 전부 외우기보다 자주 사용하면서 익힌다.

디렉터리 복사는 cp -r, 디렉터리 삭제는 rm -r이 필요하다.

rm -rf 실행 전에는 경로와 삭제 대상을 반드시 확인한다.

mv는 이동뿐 아니라 이름 변경에도 사용된다.
